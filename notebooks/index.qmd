...
This portion of the workshop introduces strategies for R processing of DNA data enriched for genomic loci, 
such as ChIPseq and ATACseq. We will discuss including common file types, 
importing peaksets, exploratory data analysis and visualizations, data transformations, and statistical
testing of differential peaks. It uses quotes and materials from [DiffBind: Differential binding analysis of ChIP-Seq peak
data](https://bioconductor.org/packages/release/bioc/vignettes/DiffBind/inst/doc/DiffBind.pdf)
by Rory Stark and Gord Brown, and workshops from [Harvard Chan Bioinformatics Core
(HBC)](https://hbctraining.github.io/Intro-to-ChIPseq/), and [CRUK Cambridge
...
4.  Plotting and reporting in DiffBind
    ...
    -   Heatmaps
    -   Profile plots
5.  Annotating Peaks and integrating with RNAseq
...
and the binding of transcription factors/histone motifiers. We will focus on 
methods that could be used to process these types of data, such as ChIPseq or 
ATACseq data.
...
In the case of ChIPseq, DNA is cross-linked and the chromatin is
fragmented and immunoprecipitated using an antibody specific to the
transcription factor of interest. The DNA captured by the
immunoprecipitation is used for library preparation and sequencing. In
the case of ATACseq, a Tn5 transposase enzyme will cut DNA and insert
sequencing adapters in open chromatin regions of the genome
('tagmentation'). These tagged fragments are sequenced and can be used
to infer regions of increased chromatin accessibility. The output from this
process is these raw reads (.fastq or .fq files), which you can then
align to the appropriate reference genome, where more
reads will align to areas of open chromatin (ATACseq) or where your protein 
of interest binds to DNA (ChIPseq). The resulting alignments can be used for 
calling using a program like MACS2/3 (https://github.com/macs3-project/MACS) or 
HOMER (http://homer.ucsd.edu/homer/ngs/peaks.html). The peaks files are used 
in downstream analyses, like differential peak calling and annotation.

![](image/chip_and_atac.png){width="80%"}
...
counting starts at 0 and you are counting the spaces between the bases.
If 1-based, you start counting at 1 and are counting each base (see
...
(https://genome.ucsc.edu/goldenPath/help/bigWig.html) here
https://hbctraining.github.io/Intro-to-ChIPseq/lectures/Fileformats.pdf.
...
https://hbctraining.github.io/Investigating-chromatin-biology-ChIPseq/lessons/07_handling_peaks_bedtools.html
...
potential protein binding sites. Each interval will have a chromosome,
start and end position, and a score indicating the strength of the peak.
It includes functions to support the processing of peak sets, including
...
differentially bound sites based on evidence of binding affinity
(measured by differences in read densities). To this end it uses
statistical routines developed in an RNA-Seq context (primarily the
...
design formula, which is discussed in greater detail later in this workshop. 
...
This will run a default DESeq2 statistical analysis (argument `method=DBA_DESEQ2`) as follows:
...
than minCount are set to minCount (default is zero). A DESeqDataSet object is then created using 
DESeq2::DESeqDataSetFromMatrix with the count matrix, metadata from the DBA object, and the design formula.
...
Displaying the results will show  differentially bound sides (DB) using a default 
...
object. We include `th=1` to include all the sites, `bNormalized = TRUE` to get 
the normalized read counts, `bCounts = TRUE` to report the count data for 
individual samples, and`bCalled = TRUE` to include information about the number 
of samples in the group identified as a peak in the original peaksets.

```{r}
tamoxifen.DB <- dba.report(tamoxifen, contrast = 1, th = 1, bNormalized = TRUE, bCounts = TRUE, bCalled = TRUE)
```

We can also set some parameters to only pull out peaks with significant changes (FDR cut off of .05 with `th = .05`).
```{r}
tamoxifen.DB.sig <- dba.report(tamoxifen, contrast = 1, th = .05, bNormalized = TRUE, bCounts = TRUE, bCalled = TRUE)
```

The results are returned as a GRanges object, but you can convert them to a data frame using `as.data.frame()`.
...
We can use the ChIPseeker package function annotatePeak to determine the structural annotations of the significant peaks.
...
For more information about running enrichment analysis, see the clusterProfiler docs: https://yulab-smu.top/biomedical-knowledge-mining-book/enrichment-overview.html

We can run Over Representation Analysis (section 5.2 in clusterProfiler documentation) on the significant peaks by getting a list of genes associated with significant peaks and then comparing to the universe of all genes associated with peaks. We use the `seq2gene` function from ChIPseeker, which says "ChIPseeker also provides seq2gene, for linking genomic regions to genes in a many-to-many mapping. It consider host gene (exon/intron), promoter region and flanking gene from intergenic region that may under control via cis-regulation. This function is designed to link both coding and non-coding genomic regions to coding genes and facilitate functional analysis."
...
GeneRatio is the ratio of k/n, where k is the overlap between your genes of interest and the genes in the gene module, and n is the total number of unique genes of interest.

If you want to see more information about which genes are contributing to these results, you can look at the results as a data frame and use dplyr::filter:
