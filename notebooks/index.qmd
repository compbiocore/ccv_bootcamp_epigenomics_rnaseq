...
4.  Git clone the repo
    (\``git clone https://github.com/compbiocore/ccv_bootcamp_annotations.git`\`)
...
This portion of the workshop introduces ChIPseq and ATACseq analysis
using R, including common file types, importing peaksets, exploratory
data analysis and visualizations, data transformations, and statistical
testing of differential peaks. It uses quotes and materials from
[DiffBind: Differential binding analysis of ChIP-Seq peak
...
In the previous section of this workshop, we discussed working with
transcriptome or RNAseq data. Transcription is a process that is
regulated by chromatin accessibility and the binding of transcription
factors/histone motifiers. We will focus on methods that could be used
to process these types of data, such as ChIPseq or ATACseq data.
...
to infer regions of increased chromatin accessibility.
...
involved in the contrast (Condition).
...
```{r}
tamoxifen.DB <- dba.report(tamoxifen, contrast = 1)
```

The results are returned as a GRanges object, but you can convert them
to a dataframe and filter the results:

```{r}
as.data.frame(tamoxifen.DB) %>% dplyr::filter(FDR <= .05) %>% dplyr::filter(Fold > 1)
```
...
```{r}
peakAnno <- annotatePeak(peak = tamoxifen.DB, TxDb=txdb, annoDb = 'org.Hs.eg.db', tssRegion=c(-1000, 1000), verbose=FALSE)
```

Here we are annotating only the significant peaks from the Tamoxifen
contrasts, but in theory we could annotate the entire consensus peakset
like this:

```{r}
consensus <- dba(tamoxifen, tamoxifen$masks$Consensus, minOverlap=1)
consensus.peaks <- dba.peakset(consensus, bRetrieve=TRUE)
consensus_peakAnno <- annotatePeak(peak = consensus.peaks, TxDb=txdb, annoDb = 'org.Hs.eg.db',  tssRegion=c(-1000, 1000), verbose=FALSE, columns = c("ENTREZID", "ENSEMBL", "SYMBOL", "GENENAME"))
...
We can use this information to run enrichment analysis:
